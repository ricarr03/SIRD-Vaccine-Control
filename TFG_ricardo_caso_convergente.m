...
% valor inicial del control para resolver el sistema
% descomentar el valor de la u para evaluar el caso deseado
u(1:M) = 0;
% u(1:M) = sin(pi*(1:M)/M);
% u(1:M) = 1;
...
% cálculo del coste sin control (para u(1:M)=0)
coste_sin = X(4,M);
for r=1:M
...
% u(1:M) = sin(pi*(1:M)/M);
u(1:M) = 0;
% contador del número de iteraciones se inicia en 0, pero la resolución
% anterior cuenta como primera iteración, en la memoria la iteración
% enterior corresponde a i=0.
iter = 0;
...
while test < 0
    ...
    % coste
    if iter>1
        if coste_control(iter)<coste_min
            xmin((1:4),(1:M)) = X((1:4),(1:M));
            umin(1:M) = oldu(1:M);
            coste_min = coste_control(iter);
            iter_min = iter;
        end
    end    
