...
clc
clf
% constantes asociadas a nuestro problema de control si se cambian afecta a
% la convergencia
B = 0.2691;
mu = 0.0121;
gamma = 0.1001;
...
% representación de la evolución de infectados sin control
% plot(t,X(2,:),'r')
plot(t,X(1,:),'r',t,X(2,:),'b',t,X(3,:),'g',t,X(4,:),'k')
% hold on
title('Evolución del sistema sin control')
% title("Evolución de (I)")
xlabel("Tiempo (días)")
ylabel('Porcentaje de individuos')
legend('S','I','R','D')
% ylabel("Infectados")
...
t(1) = d; t(M) = 400;
Y(1,1) = X(1,M); Y(2,1) = X(2,M);
Y(3,1) = X(3,M); Y(4,1) = X(4,M);

% u(1:M) = 0;
% for r=1:(M-1)
%     t(r+1) = t(r) + h;
%     F1 = F(t(r),Y(:,r),u(r));
%     F2 = F(t(r)+h/2,Y(:,r)+h*F1/2,1/2*(u(r)+u(r+1)));
%     F3 = F(t(r)+h/2,Y(:,r)+h*F2/2,1/2*(u(r)+u(r+1)));
%     F4 = F(t(r)+h,Y(:,r)+h*F3,u(r+1));
%     Y(:,r+1) = Y(:,r) + h/6 * (F1 + 2*F2 + 2*F3 + F4);
% end
% datos iniciales para la Q
Q(1,1) = 0; Q(2,1) = 0;
Q(3,1) = 0; Q(4,1) = -1;
...
%     for r=1:M
%         % cálculo del coste con control para cada iteracion
%         coste_control(iter) = coste_control(iter) + (h* (k*Y(4,r)+c/2*(Phi(Y(1,r))*u(r))^2));
%     end
    ...
    for r=1:(M-1)
        t(r+1) = t(r) + h;
        Q1 = G(t(r),Y(:,M-r+1),Q(:,r),u(M-r+1));
        Q2 = G(t(r)+h/2,1/2*(Y(:,M-r+1)+Y(:,M-r)),Q(:,r)+h*Q1/2,1/2*(u(M-r+1)+u(M-r)));
        Q3 = G(t(r)+h/2,1/2*(Y(:,M-r+1)+Y(:,M-r)),Q(:,r)+h*Q2/2,1/2*(u(M-r+1)+u(M-r)));
        Q4 = G(t(r)+h,Y(:,M-r),Q(:,r)+h*Q3,u(M-r));
        Q(:,r+1) = Q(:,r) + h/6 * (Q1 + 2*Q2 + 2*Q3 + Q4);
    end
    % cálculo de P por medio de la fórmula
    for r=1:M
    ...
    % cálculo del control
    for r=1:M
       u(r) = max(min(((Nu/N*(P(3,r)-P(1,r)))./(c*Phi(Y(1,r)))),1),0);
    end
...
title('Evolución del sistema con control recurrente','Fontsize',14)
% title('Evolución del sistema con control hasta t=200','Fontsize',14)